...
# Script to build and run the Nager MCP UI MCP Server locally in Docker
...
IMAGE_NAME="nager-mcp-ui-mcp-server"
CONTAINER_NAME="nager-mcp-ui-mcp-local"
HOST_PORT=${PORT:-8000}
CONTAINER_PORT=${PORT:-8000}

echo -e "${BLUE}🚀 Building and running Nager MCP UI MCP Server...${NC}"
echo
...
if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
        ...
        cat > .env << EOF
# Nager MCP UI MCP Server Configuration
PORT=8000
STAGE=MAINNET
LOG_LEVEL=INFO
...
echo
echo -e "${GREEN}✅ Nager MCP UI MCP Server is running!${NC}"
echo
